...
if (!interactive()) {
  ...
  args <- list(
    ...
    sample_size = 50,
    sero_over_time = "results_redacted/plot_sero_over_time/geodate2/sero_over_time_ds_geodate2.rds",
...
meta <- readRDS(meta_path) %>% dplyr::filter(filtered)
...
seros$serotype <- gsub(" ", "-", seros$serotype)

top_sero_count <- length(unique(seros$serotype))
if (length(unique(meta$serotype)) >= top_sero_count) {
  
  if ("Other" %in% seros$serotype) {
    top_s <- seros$serotype[-which(seros$serotype == "Other")][1:(top_sero_count-1)]
  } else {
    top_s <- seros$serotype[1:top_sero_count]
  }
  
  if (any(top_s %in% meta$serotype) == FALSE) {
    missing_collapsed <- paste(
      top_s[which(!top_s %in% meta$serotype)], collapse = ", ")
    stop("Serotype ", missing_collapsed," not found in metadata table.")
  }
