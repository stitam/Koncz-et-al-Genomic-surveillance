...
args_list <- list(
 ...
 make_option(
  ...
  make_option(
    ...
    type = "character",
    help = "Path to a table of geographic locations included in the study."
  )
)
...
meta_all <- readRDS(meta_path) %>% dplyr::filter(filtered)
...
countries <- read.csv(args$geographic_locations, sep = "\t") %>%
